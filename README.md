<h1 align="center">👋 Hey there, I'm Gowtham</h1>

<p align="center">
  🚀 Aspiring Cloud & DevOps Engineer | 🧠 Lifelong Learner | ⚙️ Automation Enthusiast
</p>

---

### 🌟 About Me

I'm a motivated professional transitioning from a non-IT background into the **Cloud & DevOps** space — driven by curiosity, automation, and building reliable scalable cloud systems.

---

### 🧰 Tech Stack & Tools

#### ☁️ Cloud  
<p>
<img src="https://img.shields.io/badge/AWS-232F3E?logo=amazon-aws&logoColor=white" />
<img src="https://img.shields.io/badge/CloudWatch-5A29E4?logo=amazonaws&logoColor=white" />
</p>

#### 🔧 DevOps | CI/CD  
<p>
<img src="https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white" />
<img src="https://img.shields.io/badge/Jenkins-D24939?logo=jenkins&logoColor=white" />
<img src="https://img.shields.io/badge/GitLab_CI/CD-FC6D26?logo=gitlab&logoColor=white" />
<img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white" />
<img src="https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white" />
</p>

#### 🏗️ Infrastructure as Code  
<p>
<img src="https://img.shields.io/badge/Terraform-7B42BC?logo=terraform&logoColor=white" />
<img src="https://img.shields.io/badge/Ansible-EE0000?logo=ansible&logoColor=white" />
</p>

#### 📡 Monitoring & Automation  
<p>
<img src="https://img.shields.io/badge/Prometheus-E6522C?logo=prometheus&logoColor=white" />
<img src="https://img.shields.io/badge/Grafana-F46800?logo=grafana&logoColor=white" />
<img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/Shell_Script-121011?logo=gnu-bash&logoColor=white" />
</p>

---

### 🧪 Hands-On Experience

I've built real-world end-to-end DevOps projects involving:

- 🚀 Automated deployments using **Jenkins**
- 🏗️ Infrastructure provisioning via **Terraform**
- 📦 Containerizing apps using **Docker + Kubernetes**
- ⚙️ Shell/Python scripts for automation and configurations
- 📊 Monitoring using **Prometheus & Grafana**

These helped me build a strong understanding of **CI/CD pipelines, IaC, orchestration, and observability**.

---

### 🎯 Current Goal

📍 Becoming a full-time **Cloud/DevOps Engineer**  
📍 Collaborating with real teams, solving real problems  
📍 Keep learning, improving and contributing to open-source  

---

### 📈 GitHub Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Dayith&show_icons=true&theme=tokyonight&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Dayith&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

### 📬 Connect With Me

<p align="left">
<a href="https://www.linkedin.com/in/YOUR-LINK-HERE/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white" /></a>
<a href="mailto:YOUR_EMAIL_HERE"><img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white" /></a>
</p>

---

⭐ **Thanks for visiting — feel free to check out my repositories and projects!**
